...
import { Space, Typography, Tag, Modal, Form, Input, Select, message } from 'antd';
...
export default function LossAnalysisPage() {
  ...
  const columns = [
    {
      ...
      width: 180,
      render: (val: string) => <Text className="text-slate-900 font-bold">{val}</Text>,
    },
    {
      title: '流失风险信息',
      width: 250,
      render: (_: any, record: any) => (
        <div className="flex flex-col gap-1">
          <Text className="text-slate-900 font-bold">流失原因: {record.loss_reason || '-'}</Text>
          <Text className="text-slate-500 font-medium text-xs">商品: {record.product_name || '未关联'}</Text>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (val: string) => {
        ...
        return <Tag color={st.color} className="font-bold">{st.text}</Tag>;
      },
    },
    {
      title: '挽回备注',
      dataIndex: 'recovery_remark',
      render: (val: string) => <Text className="text-slate-600 font-medium truncate max-w-[200px]" title={val}>{val || '-'}</Text>,
    },
    {
      title: '生成时间',
  ...
  return (
    <div className="p-6 h-full flex flex-col">
        ...
        searchRender={(formProps) => (
          <Form {...formProps} layout="inline" className="w-full flex">
            ...
            <Space className="ml-auto">
              <a className="ant-btn ant-btn-default font-bold text-slate-700 hover:text-slate-900 border-slate-300" onClick={() => formProps.form?.resetFields()}>重置</a>
              <a className="ant-btn ant-btn-primary font-black bg-slate-900" onClick={() => formProps.form?.submit()}>查询</a>
            </Space>
          </Form>
        )}
      />
      ...
      <Modal
        title={<span className="text-slate-900 font-black">流失挽回跟进记录</span>}
        open={modalVisible}
        onOk={handleSave}
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          <Form.Item label={<Text className="text-slate-900 font-bold">跟进状态</Text>} name="recovery_state" rules={[{ required: true, message: '请选择状态' }]}>
            <Select>
              <Select.Option value="pending">待处理</Select.Option>
          ...
          <Form.Item label={<Text className="text-slate-900 font-bold">处理备注</Text>} name="recovery_remark">
            <TextArea rows={4} placeholder="记录客服致电或二次沟通的情况，方便复盘" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
