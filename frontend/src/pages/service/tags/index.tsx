...
export default function QualityTagsPage() {
      ...
      if (action === "confirm") {
        await confirmBatchAction(
          ...
          async () => {
            ...
            message.success(
              `已成功确认并入库 ${selectedRowKeys.length} 条智能标签`,
            );
            setSelectedRowKeys([]);
            tableRef.current?.reload();
          },
  ...
  const handleDedup = async () => {
    try {
      ...
      const sourceTagsInput = values.source_tags
        .split(",")
        .map((s: string) => s.trim())
        .filter(Boolean);
      if (sourceTagsInput.length === 0)
        return message.warning("请输入需要被合并的原标签");
    ...
    <div className="p-6 h-full flex flex-col pt-2">
        ...
        items={[
          {
            key: "pending",
            label: (
              <span className="font-bold text-base px-2">待复审确认池</span>
            ),
          },
          {
            key: "confirmed",
            label: (
              <span className="font-bold text-base px-2 text-emerald-600">
                已生效/库内标签
              </span>
            ),
          },
          {
            key: "rejected",
            label: (
              <span className="font-bold text-base px-2 text-slate-400">
                已驳回阻截
              </span>
            ),
          },
      ...
      <div className="flex-1 bg-white border border-slate-200">
        <BaseTable
          ref={tableRef}
          request={(params) =>
            serviceApi.queryQualityTags({ ...params, status: activeKey })
          }
          columns={columns}
          rowKey="id"
          hideSearch
              ...
              <Space className="py-2 bg-slate-50 px-4 rounded border border-slate-200 w-full animate-fade-in shadow-sm">
                ...
                <Permission code="service:tag:audit">
                  <Button
                    ...
                    className="bg-slate-900 font-black text-white hover:bg-slate-800 ml-4 border-black"
                  >
                    批量通过并释放入库
                  </Button>
                  <Button
                    ...
                    className="font-bold bg-white ml-2"
                  >
                    直接物理驳回
                  </Button>
                </Permission>
      ...
      <Modal
        ...
        okButtonProps={{
          className:
            "bg-rose-600 text-white font-black border-transparent hover:bg-rose-700",
        }}
      ...
      >
        <div className="bg-amber-50 text-amber-800 p-4 rounded mb-6 border border-amber-200 font-bold">
          警告提示:
          该操作不可逆，将把底层的杂乱源标签统一修正挂载到目标标准分类标签下，以降低图谱复杂度负荷并提升召回精度。
        </div>
        <Form form={dedupForm} layout="vertical">
          <Form.Item
            label={
              <Text className="text-slate-900 font-bold">标准目标标签字面</Text>
            }
            name="target_tag"
            rules={[{ required: true }]}
          >
            <Input
              placeholder="输入清洗后准备保留的准确标签名"
              size="large"
          ...
          >
            <Input.TextArea
              placeholder="例如: 物流太慢,快递超级无语,这物流..."
              rows={3}
