...
export default function ServiceSessionsPage() {
  ...
  const [riskView, setRiskView] = useState<'all' | 'high' | 'medium' | 'low'>('all');
  ...
  const columns: ProColumns<ServiceSessionRecord>[] = [
    { title: '会话编号', dataIndex: 'session_no', width: 180, className: 'leixi-text-main' },
    {
      title: '客户 / 客服',
      render: (_, record) => (
        <Space direction="vertical" size={0}>
          <Text className="leixi-text-main font-bold">{record.customer_nickname || '匿名客户'}</Text>
          <Text className="leixi-text-secondary text-xs">{record.agent_name || '-'}</Text>
        </Space>
      )
    },
    {
      ...
      render: (_, record) => {
        ...
        return (
          <Space direction="vertical" size={2}>
            <Tag color={analysis.quality_passed ? 'success' : 'error'}>{analysis.quality_passed ? '合格' : '不合格'}</Tag>
            <Text className="leixi-text-secondary text-xs">分 {analysis.quality_score}</Text>
          </Space>
        );
      }
    },
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button type="link" onClick={() => navigate(`/service/sessions/${record.id}`)}>详情</Button>
          <Permission code="service:quality:analyze">
            <Button type="link" loading={analyzeMutation.isPending} onClick={() => analyzeMutation.mutate(record.id)}>重分析</Button>
          </Permission>
        </Space>
      )
    }
