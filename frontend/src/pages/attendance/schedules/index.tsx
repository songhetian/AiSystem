...
export default function AttendanceSchedulesPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () =>
      attendanceApi.getDashboard({
        ...
        ...(employeeKeyword ? { keyword: employeeKeyword } : {}),
      }),
  });
  ...
  const employeeOptions = useMemo(
    () =>
      employees
        .filter((item: { department_id?: string }) => (!deptId ? true : item.department_id === deptId))
  ...
  return (
    <div className="leixi-page-container bg-slate-50">
      <ScheduleFilterBar
        ...
        onPrevMonth={() => setMonth((current) => current.subtract(1, 'month'))}
      ...
      <div className="mb-4 grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        <Card className="rounded-2xl border-slate-200 shadow-sm" bodyStyle={{ padding: 18 }}>
          <div className="mb-4">
            <Title level={5} className="!mb-1">
              班次列表
            </Title>
            <Text type="secondary">点击激活班次，拖拽或点击表格单元格即可排班。</Text>
          </div>
            ...
            {data?.shifts?.map((shift: AttendanceScheduleShift) => (
              <DraggableShiftCard
                ...
                onSelect={() => {
                  ...
                  setSelectedShiftId((current) => (current === shift.id ? undefined : shift.id));
                }}
              />
            ))}
          </Space>
          ...
          <Layout className="bg-transparent">
            <Layout.Content>
              <Card
                ...
                bodyStyle={{ padding: 0 }}
                extra={saving ? <span className="text-xs text-slate-500">保存中...</span> : null}
              >
                <div className="border-b border-slate-200 bg-slate-50 px-5 py-3">
                  <Space direction="vertical" size={0}>
                    <Text strong>{month.format('YYYY 年 MM 月排班看板')}</Text>
                    <Text type="secondary">
                      {scheduleMode
                        ? selectedShift
