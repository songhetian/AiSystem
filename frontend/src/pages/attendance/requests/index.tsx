...
import { useAttendanceRequests, type RequestType } from "./hooks/useAttendanceRequests";
  ...
  return (
    <div className="leixi-page-container">
      <Card className="shadow-sm" bodyStyle={{ padding: 24 }}>
        <Tabs
          items={[
            { key: 'leave', label: '请假申请', children: <RequestTabContent type="leave" /> },
            { key: 'overtime', label: '加班申请', children: <RequestTabContent type="overtime" /> },
            { key: 'patch-card', label: '补卡申请', children: <RequestTabContent type="patch-card" /> },
            { key: 'schedule-change', label: '调班申请', children: <RequestTabContent type="schedule-change" /> },
          ]}
        />
      </Card>
    </div>
