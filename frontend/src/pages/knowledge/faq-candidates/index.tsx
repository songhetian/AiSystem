...
export default function KnowledgeFaqCandidatesPage() {
  ...
  useKeyboardShortcuts({
    "Ctrl+r": () => {
      queryClient.invalidateQueries({ queryKey: ["knowledge-faq-candidates"] });
      message.success("已刷新");
    },
  });
  ...
  const columns: ProColumns<KnowledgeFaqCandidate>[] = [
    ...
    { title: "最近分析时间", dataIndex: "latest_analyzed_at", width: 180 },
    {
      ...
      width: 120,
      render: (_, record) => (
        <Tag color={record.already_archived ? "success" : "warning"}>
          {record.already_archived ? "已沉淀" : "待沉淀"}
        </Tag>
      ),
    },
    {
  ...
  return (
    ...
    >
      <GlobalLoading loading={isLoading}>
        <BaseTable<KnowledgeFaqCandidate>
          rowKey="question"
          columns={columns}
          dataSource={data}
          loading={isLoading}
