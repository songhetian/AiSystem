...
import { Divider, Form, Input, Select, Space, Tag, Typography, Button, Card, message } from 'antd';
import { KeyOutlined, PlusOutlined, ReloadOutlined, SafetyCertificateOutlined, SearchOutlined } from '@ant-design/icons';
...
export default function ApiKeysPage() {
  ...
  const { data: apiKeys = [], isLoading } = useQuery({ queryKey: ['system-api-keys'], queryFn: systemApi.listApiKeys });
  ...
  const filteredApiKeys = useMemo(() => {
    ...
    return apiKeys.filter((item) => item.name.toLowerCase().includes(keyword.toLowerCase()));
  }, [apiKeys, keyword]);

  const refresh = () => queryClient.invalidateQueries({ queryKey: ['system-api-keys'] });
  ...
  const columns: ProColumns<ApiKeyRecord>[] = [
    ...
    },
    {
      ...
      render: (value) => <Tag className="border-slate-300 font-bold uppercase">{value}</Tag>,
    },
    {
      title: 'API Key',
      dataIndex: 'api_key',
      render: (value) => <Text className="font-mono text-slate-400">{String(value).slice(0, 6)}****************{String(value).slice(-4)}</Text>,
    },
    {
      title: '分配范围',
      dataIndex: 'dept_id',
      render: (deptId, record) => {
        const platform = platforms.find((item) => item.id === record.platform_id);
        const department = departments.find((item) => item.id === deptId);
        return (
          <Space direction="vertical" size={0}>
            <Text className="text-xs text-slate-500">{platform?.name || '-'}</Text>
    ...
    },
    {
      ...
      dataIndex: 'status',
      render: (value) => (
        <Tag color={value === 1 ? 'success' : 'default'} className="border-2 font-bold">
          {value === 1 ? '启用' : '禁用'}
        </Tag>
      ),
    },
  ...
  return (
    <div className="space-y-4 p-4">
      <Card bordered={false} className="shadow-sm">
        <Form form={filterForm} layout="inline" className="flex flex-wrap items-center gap-4">
          <Form.Item name="name" className="mb-0 min-w-[200px] flex-grow">
            <Input prefix={<SearchOutlined />} placeholder="搜索凭据名称" className="h-[44px]" />
          </Form.Item>
      ...
      <Card bordered={false} className="shadow-sm">
        <BaseTable<ApiKeyRecord> columns={columns} dataSource={filteredApiKeys} rowKey="id" loading={isLoading} />
      </Card>
      ...
      >
        <Form form={form} layout="vertical" initialValues={{ status: 1 }} onFinish={(values) => saveMutation.mutate(values)}>
          <Form.Item label="凭据名称" name="name" rules={[{ required: true, message: '请输入凭据名称' }]}>
            <Input placeholder="如：OpenAI 官方 Key" className="font-bold text-slate-900" />
          </Form.Item>
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <Form.Item label="分配部门（可选）" name="dept_id" tooltip="留空表示该平台下所有部门共享此 Key">
                <Select allowClear placeholder="全局共享" options={departments.map((item) => ({ label: item.name, value: item.id }))} />
              </Form.Item>
            </div>
          </div>
        </Form>
