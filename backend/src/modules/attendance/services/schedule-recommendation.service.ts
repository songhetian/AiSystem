...
export class ScheduleRecommendationService {
  ...
  async generateRecommendations(
    ...
    this.logger.log(
      `生成排班推荐: 部门=${deptId}, 周期=${startDate}~${endDate}`,
    );
    ...
    // 2. 生成参数推荐
    const paramRecommendations =
      this.generateParamRecommendations(historicalData);
    ...
    // 4. 生成人力需求推荐
    const demandRecommendations = await this.generateDemandRecommendations(
      platformId,
      deptId,
      startDate,
      endDate,
  ...
  private async analyzeHistoricalData(platformId: string, deptId: string) {
    ...
    const avgComplianceRate =
      histories.reduce((sum, h) => sum + h.compliance_rate, 0) / totalRecords;
    const avgSatisfactionRate =
      histories.reduce((sum, h) => sum + h.satisfaction_rate, 0) / totalRecords;
    const avgFittingRate =
      histories.reduce((sum, h) => sum + h.fitting_rate, 0) / totalRecords;
    const avgWarningCount =
      histories.reduce((sum, h) => sum + h.warning_count, 0) / totalRecords;

    // 找出最佳模式
    const modeStats = new Map<
      string,
      { count: number; avgCompliance: number }
    >();
  ...
  private generateParamRecommendations<T extends Record<string, any>>(
    ...
    if (historicalData.avgWarningCount > 10) {
      ...
      recommendations._reason =
        "历史数据显示警告较少，可以收紧限制以提高员工满意度";
    }
   ...
   * 生成班次推荐
   */
  private async generateShiftRecommendations(
    platformId: string,
    deptId: string,
    historicalData: any,
  ) {
    ...
    const recommendations = shifts.map((shift) => {
      ...
      const avgUsage =
        Array.from(shiftUsage.values()).reduce((a, b) => a + b, 0) /
        shiftUsage.size;
    ...
    for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
      ...
      for (const [shiftName, stats] of demandStats.entries()) {
        ...
        recommendations.push({
          ...
          confidence:
            stats.count >= 4 ? "high" : stats.count >= 2 ? "medium" : "low",
        });
      }
    }
  ...
  private calculateImprovements<T extends Record<string, any>>(
    ...
    const improvements: Array<Record<string, any>> = [];

    if (
      optimizedConfig.max_consecutive_days !==
      originalConfig.max_consecutive_days
    ) {
      improvements.push({
    ...
    if (
      optimizedConfig.max_hours_per_week !== originalConfig.max_hours_per_week
    ) {
      improvements.push({
