...
export class AttendanceWorkflowsService {
  private readonly approvalLockTtlSeconds = Number(
    process.env.ATTENDANCE_APPROVAL_LOCK_TTL_SECONDS ?? 30,
  );
  ...
  private async resolveEmployees(
    userId: string,
    query: QueryAttendanceRecordsDto | QueryAttendanceWorkflowsDto,
  ) {
  ...
  private attachEmployeeInfo<T extends { employee_id: string }>(
    items: T[],
    employees: any[],
  ) {
    ...
    return items.map((item) => {
      ...
      return {
        ...
        platform_id:
          item["platform_id" as keyof T] ?? employee?.platform_id ?? null,
      };
    });
  }

  private getDelegate(type: WorkflowType) {
    const tableName = WORKFLOW_CONFIG[type].delegate;
    return this.prisma[tableName as keyof typeof this.prisma] as any;
  }
  ...
  private async notifyScheduleChange(
    ...
    for (const recipientId of recipientIds) {
      ...
      await this.messageService.send({
        ...
        messageType: isAffectedEmployee
          ? "schedule_change_notified"
  ...
  private ensureWorkflowMutable(
    approvalStatus: number | null | undefined,
    entityName: string,
  ) {
    if (approvalStatus === 1) {
      throw new BadRequestException(
        `${entityName}已审批通过，不允许再修改或删除`,
      );
    }
  }
  ...
  private async ensureNoDuplicateLeave(
    employeeId: string,
    dto: UpsertAttendanceLeaveDto,
    excludeId?: string,
  ) {
  ...
  private async ensureNoOverlappingLeave(
    employeeId: string,
    startTime: Date,
    endTime: Date,
    excludeId?: string,
  ...
  private async ensureNoDuplicateOvertime(
    employeeId: string,
    dto: UpsertAttendanceOvertimeDto,
    excludeId?: string,
  ) {
  ...
  private async ensureNoOverlappingOvertime(
    employeeId: string,
    startTime: Date,
    endTime: Date,
    excludeId?: string,
  ...
  private async ensureNoDuplicatePatchCard(
    employeeId: string,
    dto: UpsertAttendancePatchCardDto,
    excludeId?: string,
  ) {
  ...
  private async ensureNoDuplicateScheduleChange(
    employeeId: string,
    dto: UpsertAttendanceScheduleChangeDto,
    excludeId?: string,
  ) {
  ...
  private async ensureLeaveOvertimeConflictFree(
    employeeId: string,
    startTime: Date,
    endTime: Date,
  ) {
  ...
  private async ensureOvertimeLeaveConflictFree(
    employeeId: string,
    startTime: Date,
    endTime: Date,
  ) {
  ...
  async listWorkflow(
    type: WorkflowType,
    userId: string,
    query: QueryAttendanceWorkflowsDto,
  ) {
    const { scope, employees } = await this.resolveEmployees(userId, query);
    if (employees.length === 0) {
      return [];
    ...
    const items = await delegate.findMany({
      where: this.scopeService.applyScope(
        scope,
        {
          is_deleted: 0,
          employee_id: { in: employees.map((item) => item.id) },
          ...(query.approval_status !== undefined
  ...
  async createLeave(userId: string, dto: UpsertAttendanceLeaveDto) {
    ...
    const { employee } = await this.resolveEmployeeForWrite(
      userId,
      dto.employee_id,
    );
    ...
    const created = await this.prisma.attendance_leave.create({
      data: {
        leave_no:
          dto.leave_no || buildSerial(WORKFLOW_CONFIG.leave.serialPrefix),
        ...
        approved_time: dto.approved_time
          ? new Date(dto.approved_time)
          : undefined,
  ...
  async updateLeave(userId: string, id: string, dto: UpsertAttendanceLeaveDto) {
    const { startTime, endTime } = this.validateLeaveDto(dto);
    const current = await this.prisma.attendance_leave.findUnique({
      where: { id },
    });
  ...
  async createOvertime(userId: string, dto: UpsertAttendanceOvertimeDto) {
    ...
    const created = await this.prisma.attendance_overtime.create({
      data: {
        overtime_no:
          dto.overtime_no || buildSerial(WORKFLOW_CONFIG.overtime.serialPrefix),
        employee_id: employee.id,
        start_time: startTime,
        end_time: endTime,
        duration_hours: dto.duration_hours,
  ...
  async updateOvertime(
    ...
    const { startTime, endTime } = this.validateOvertimeDto(dto);
    const current = await this.prisma.attendance_overtime.findUnique({
      where: { id },
    });
    if (!current || current.is_deleted) {
  ...
  async createPatchCard(userId: string, dto: UpsertAttendancePatchCardDto) {
    ...
    const created = await this.prisma.attendance_patch_card.create({
      data: {
        patch_no:
          dto.patch_no ||
          buildSerial(WORKFLOW_CONFIG["patch-card"].serialPrefix),
        employee_id: employee.id,
  ...
  async updatePatchCard(
    userId: string,
    id: string,
    dto: UpsertAttendancePatchCardDto,
  ) {
    const { patchDate, targetTime } = this.validatePatchCardDto(dto);
    const current = await this.prisma.attendance_patch_card.findUnique({
      where: { id },
    });
    if (!current || current.is_deleted) {
  ...
  async createScheduleChange(
    ...
    const created = await this.prisma.attendance_schedule_change.create({
      data: {
        change_no:
          dto.change_no ||
          buildSerial(WORKFLOW_CONFIG["schedule-change"].serialPrefix),
        employee_id: employee.id,
  ...
  async updateScheduleChange(
    ...
    const { changeDate } = this.validateScheduleChangeDto(dto);
    const current = await this.prisma.attendance_schedule_change.findUnique({
      where: { id },
    });
    if (!current || current.is_deleted) {
