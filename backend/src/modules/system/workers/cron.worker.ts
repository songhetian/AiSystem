...
export class SystemCronWorker {
  ...
  async handleIntegrationTick() {
    ...
    try {
      ...
      this.logger.error(`Integration cron tick failed: ${message}`);
    } finally {
  ...
  async handleLogCleanup() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Maintenance failed: ${message}`);
    }
  }
  ...
  async handleMessageRetention() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Message retention cleanup failed: ${message}`);
    }
  }
  ...
  private async runJob(job: any) {
    ...
    try {
      ...
      // 2. 数据处理流... (TODO: 需要实现 fetchData 方法)
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`Job [${job.name}] failure: ${message}`);
  ...
  private parseNextOccurrence(cron: string): Date {
    try {
      ...
      this.logger.error(`Invalid Cron Expression [${cron}]: ${err instanceof Error ? err.message : String(err)}`);
      // 容错：默认为 1 小时后
