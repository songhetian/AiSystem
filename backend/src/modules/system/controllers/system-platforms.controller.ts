...
export class SystemPlatformsController {
  constructor(
    private readonly systemPlatformsService: SystemPlatformsService,
  ) {}
  ...
  findAll(
    ...
    return this.systemPlatformsService.findAll(user.sub, pagination);
  }
  ...
  batchUpdateStatus(
    @CurrentUser() user: CurrentUserPayload,
    @Body() dto: BatchUpdatePlatformStatusDto,
  ) {
    return this.systemPlatformsService.batchUpdateStatus(
      user.sub,
      dto.ids,
      dto.status,
    );
