...
export class MappingService {
   ...
   */
  async getMergedRules(templateId: string): Promise<{ rules: Record<string, string>, cleaning: Record<string, string> }> {
    const template = await (this.prisma as any).sys_mapping_template.findUnique({
  ...
  private mapSingle(item: any, rules: Record<string, string>, cleaning: Record<string, string>) {
    ...
    for (const [stdField, externalPath] of Object.entries(rules)) {
      try {
        ...
        this.logger.warn(`Field ${stdField} mapping failed for path ${externalPath}`);
   ...
   * 标准化订单入库 (Section 4.3)
   */
  async upsertOrders(platformId: string, shopId: string | null, deptId: string, mappedOrders: any[]) {
    const results = { created: 0, updated: 0, failed: 0 };
    for (const order of mappedOrders) {
      try {
        await (this.prisma as any).bi_order.upsert({
          where: {
          ...
          create: {
            ...
            address: order.address,
            order_time: order.order_time ? new Date(order.order_time) : new Date(),
            raw_data: order
          }
        });
        results.updated++;
   ...
   */
  async upsertProducts(platformId: string, shopId: string | null, deptId: string, mappedProducts: any[]) {
    const results = { created: 0, updated: 0, failed: 0 };
    for (const prod of mappedProducts) {
      try {
        await (this.prisma as any).bi_product.upsert({
          where: {
