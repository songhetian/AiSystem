...
export class PlatformIntegrationAdapterService {
  ...
  async *fetchDataStream(jobType: string, config: PlatformConfigRecord, maxPages = 50) {
    if (!config.api_endpoint) throw this.buildError('Endpoint Missing', IntegrationErrorCode.API_CALL_ERROR);
    ...
    while (hasMore && currentPage <= maxPages) {
      ...
      this.logger.log(`🔄 启动同步任务 [${jobType}] | 平台: ${config.platform_id} | Key: ${config.app_key?.slice(0, 4)}****`);
      ...
      // 2. 字段映射标准化
      const standardizedItems = rawItems.map(item => this.mapToStandardSchema(item, config.mapping_rules));
  ...
  private async executeRequestWithRetry(config: PlatformConfigRecord, page: number, retries = 3): Promise<any> {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        // 规范 2.2.1: 校验业务状态码
        if (resJson.code !== 200) throw new Error(resJson.message || '业务逻辑异常');
