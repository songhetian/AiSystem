...
export class DashboardService {
  ...
  async copyTemplate(id: string, userId: string) {
    const original = await this.prisma.sys_dashboard_template.findUnique({ where: { id } });
  ...
  async getGlobalOverview(userId: string) {
    ...
    const [userCount, deptCount, shopCount, orderSum, reimSum, sessionCount] = await Promise.all([
      ...
      (this.prisma as any).service_session.count({ where }),
    ]);
  ...
  async getInterfaceMonitoring(userId: string) {
    const scope = await this.scopeService.resolveAccess(userId);
    const stats = await (this.prisma as any).sys_integration_stat.findMany({
      where: {
    ...
    return stats.map((item: any) => ({
      ...
      successRate: item.total_calls > 0 ? (item.success_calls / item.total_calls) * 100 : 100,
      failCount: item.fail_calls,
      totalCount: item.total_calls,
      status: item.fail_calls > 5 || item.avg_duration_ms > 1000 ? 'abnormal' : 'normal',
    }));
  }
  ...
  async getServiceOverview(userId: string) {
    ...
    const [sessionCount, analysisStats, satisfactionStats, passCount] = await Promise.all([
      (this.prisma as any).service_session.count({ where }),
      (this.prisma as any).service_session_analysis.aggregate({
        where,
        _avg: { quality_score: true, response_timeout_count: true },
        _count: { id: true },
      }),
      (this.prisma as any).service_satisfaction.aggregate({
        where,
        _avg: { rating: true },
        _count: { id: true },
      }),
      // V7.0 实战化：统计真实的质检合格数 (假设 status=1 为合格)
      (this.prisma as any).service_session_analysis.count({
        where: { ...where, status: 1 }
      })
    ]);

    // 计算真实的合格率
    const realPassRate = analysisStats._count.id > 0 
    ...
    return {
      ...
      averageQualityScore: Number((analysisStats._avg.quality_score || 0).toFixed(1)),
      averageResponseTime: Number((analysisStats._avg.response_timeout_count || 0).toFixed(1)), 
  ...
  async getHrOverview(userId: string) {
    ...
    const [activeCount, leaveCount] = await Promise.all([
      ...
      (this.prisma as any).attendance_leave.count({ where: { ...where, approval_status: 1 } }),
    ]);
  ...
  async recordAlert(userId: string, data: any) {
     ...
     // 1. 持久化记录
     const record = await (this.prisma as any).sys_dashboard_alert_record.create({
       data: {
         ...data,
         platform_id: scope.platform_id,
         dept_id: scope.dept_id,
     ...
     if (now - lastTrigger > 3600000) { // 1 Hour
  ...
  async listAlertHistory(userId: string) {
    const scope = await this.scopeService.resolveAccess(userId);
    return (this.prisma as any).sys_dashboard_alert_record.findMany({
