...
export class IntegrationMonitorService {
  ...
  async aggregateLogsToStats(platformId?: string, deptId?: string) {
    ...
    for (const group of scopeGroups.values()) {
      await (this.prisma as any).sys_integration_stat.upsert({
        where: {
    ...
    this.logger.log(`Aggregation completed. Processed ${scopeGroups.size} scope groups.`);
  }
  ...
  async getHealthReport(platformId: string, deptId: string, shopId?: string) {
    ...
    const avgDuration = Math.round(stats.reduce((sum, s) => sum + s.avg_duration_ms, 0) / stats.length);
