...
import { DISTRIBUTED_LOCK_KEY, DistributedLockOptions } from '../decorators/distributed-lock.decorator';
...
export class BusinessLockInterceptor implements NestInterceptor {
  ...
  async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<unknown>> {
    ...
    const options = this.reflector.getAllAndOverride<DistributedLockOptions>(DISTRIBUTED_LOCK_KEY, [handler, controller]);
    if (!options) {
      return next.handle();
    }
    ...
    const resultPromise = this.businessLockService.runExclusive(resolvedKey, ttl, async () => {
